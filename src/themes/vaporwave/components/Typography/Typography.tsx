...
const gradientVariants = new Set<TypographyVariant>(["display", "h1"]);

const glowVariants = new Set<TypographyVariant>(["h2", "h3", "label"]);
...
export function Typography({
  ...
  const useGradient = gradientVariants.has(variant) && !muted;
  const useGlow = glowVariants.has(variant) && !muted;
    ...
    <Tag
      className={cn(
        ...
        muted
          ...
          : useGradient
            ? "bg-gradient-to-r from-[#FF9900] via-[#FF00FF] to-[#00FFFF] bg-clip-text text-transparent"
